	...
	const updateTodo = (id, newDescription, newEndDate) => {
		...
		todos.map((todo) =>
			todo.id === id
			? { ...todo, description: newDescription, end_date: newEndDate }
			: todo
		)
		)
	}
	...
	const filteredTodos = todos.filter((todo) => {
		const matchesSearsh = todo.description
		.toLowerCase()
	...
	return (
		...
		<div style={{ display: "flex", flexDirection: "row", gap: "20px" }}>
			...
			<Tab
				eventKey="active"
				title={`Active (${
				todos.filter((task) => !task.completed && !task.archived).length
				})`}
			/>
			<Tab
				eventKey="completed"
			...
			<Tab
				eventKey="archived"
				title={`Archived (${todos.filter((task) => task.archived).length})`}
			/>
			</Tabs>
